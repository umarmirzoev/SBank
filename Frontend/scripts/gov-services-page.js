...
async function loadData(elements) {
  try {
    ...
    console.error(error);
    elements.providersList.innerHTML = `<div class="empty-state">Не удалось загрузить госуслуги.</div>`;
    elements.recentPayments.innerHTML = `<div class="empty-state">История госуслуг пока недоступна.</div>`;
    showToast("Не удалось загрузить страницу госуслуг");
  }
}
...
async function loadProviders(elements) {
  const govCategory = state.categories.find((item) => String(item.Code || item.code || "").toLowerCase() === "fines_taxes");
  if (!govCategory) {
    state.providers = [];
    renderProviders(elements);
    return;
  }
  const payload = await apiRequest(`/api/BillPayment/categories/${govCategory.Id || govCategory.id}/providers`, { auth: true });
...
function resolvePresetProviderId(providers) {
  const preset = sessionStorage.getItem(GOV_PROVIDER_PRESET_KEY);
  ...
  sessionStorage.removeItem(GOV_PROVIDER_PRESET_KEY);
...
function providerVisual(provider) {
  ...
  if (code.includes("traffic")) return { icon: "🛡", subtitle: "Штрафы и ГИБДД" };
  if (code.includes("income")) return { icon: "🧾", subtitle: "Налоги" };
  return { icon: "🏛", subtitle: "Госуслуга" };
}

function renderProviders(elements) {
  if (!state.providers.length) {
    elements.providersList.innerHTML = `<div class="empty-state">Для госуслуг пока нет доступных поставщиков.</div>`;
    return;
  }
...
function renderPopularServices(elements) {
  if (!state.providers.length) {
    elements.popularServices.innerHTML = `<div class="empty-state">Популярные госуслуги появятся после загрузки поставщиков.</div>`;
    return;
  }
...
function renderRecentPayments(elements) {
  const govPayments = state.payments.filter((payment) => String(payment.CategoryName || payment.categoryName || "").toLowerCase().includes("fines"));
  if (!govPayments.length) {
    elements.recentPayments.innerHTML = `<div class="empty-state">У вас пока нет реальных платежей по госуслугам.</div>`;
    return;
  }
  elements.recentPayments.innerHTML = govPayments.slice(0, 6).map((payment) => {
...
async function submitPayment(elements) {
  ...
  if (!providerId) return showToast("Выберите госуслугу");
  if (!accountNumber) return showToast("Введите ИНН или номер постановления");
  ...
  try {
    await apiRequest("/api/BillPayment/pay", {
      method: "POST",
      auth: true,
      body: { accountId, providerId, accountNumber, amount, currency, description: `Оплата ${provider?.Name || provider?.name || "госуслуги"}` }
    });
