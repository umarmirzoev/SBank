(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function a(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(s){if(s.ep)return;s.ep=!0;const i=a(s);fetch(s.href,i)}})();const b="sbank-session";let n,m=null;function A(e){try{return JSON.parse(e)}catch{return null}}function d(){return A(localStorage.getItem(b)||"null")}function v(){return!!d()?.token}function q(e){localStorage.setItem(b,JSON.stringify(e)),window.dispatchEvent(new CustomEvent("sbank:session-changed",{detail:e}))}function y(e=!1){localStorage.removeItem(b),window.dispatchEvent(new CustomEvent("sbank:session-changed",{detail:null})),e&&c("Вы вышли из аккаунта.")}function L(e,t){if(!e)return t;if(Array.isArray(e.Description)&&e.Description.length>0)return e.Description.join(" ");if(e.title)return e.detail?`${e.title} ${e.detail}`:e.title;if(e.errors){const a=Object.values(e.errors).flat();if(a.length>0)return a.join(" ")}return typeof e=="string"?e:t}async function g(e,t={}){const{method:a="GET",body:r,auth:s=!1,headers:i={}}=t,o=d(),u=await fetch((S=>{const l=String(S||"").trim();if(!l||/^https?:\/\//i.test(l))return l;const N=String(T||"").replace(/\/$/,""),E=l.startsWith("/")?l:`/${l}`;return`${N}${E}`})(e),{method:a,headers:{"Content-Type":"application/json",...s&&o?.token?{Authorization:`Bearer ${o.token}`}:{},...i},body:r===void 0?void 0:JSON.stringify(r)}),f=(u.headers.get("content-type")||"").includes("application/json")?await u.json():await u.text();if(!u.ok)throw u.status===401&&y(),new Error(L(f,`Ошибка запроса: ${u.status}`));return f}function p(e){return e&&typeof e=="object"&&"StatusCode"in e?{data:e.Data,messages:Array.isArray(e.Description)?e.Description:[],statusCode:e.StatusCode}:{data:e,messages:[],statusCode:200}}function D(e,t=""){const a=Number(e||0),r=a.toLocaleString("ru-RU",{minimumFractionDigits:a%1===0?0:2,maximumFractionDigits:2});return t?`${r} ${t}`:r}function O(e){return e?new Date(e).toLocaleString("ru-RU",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"Не указано"}function c(e){const t=document.getElementById("toast");t&&(t.textContent=e,t.classList.add("show"),clearTimeout(c.timer),c.timer=setTimeout(()=>t.classList.remove("show"),2200))}function k(){if(n)return n;n=document.createElement("div"),n.className="sb-modal-backdrop",n.hidden=!0,n.innerHTML=`
  ...
  `,document.body.appendChild(n),n.addEventListener("click",r=>{(r.target===n||r.target.closest("[data-auth-close]"))&&h()});const e=n.querySelectorAll("[data-auth-form]"),t=n.querySelectorAll("[data-auth-tab]"),a=n.querySelector("[data-auth-message]");return t.forEach(r=>{r.addEventListener("click",()=>{const s=r.dataset.authTab;t.forEach(i=>i.classList.toggle("active",i===r)),e.forEach(i=>{i.hidden=i.dataset.authForm!==s}),a.className="sb-message info",a.textContent=s==="login"?"Введите email и пароль от backend-сервиса.":"Регистрация создаст нового пользователя через API /api/auth/register."})}),n.querySelector('[data-auth-form="login"]').addEventListener("submit",async r=>{r.preventDefault();const s=r.currentTarget,i=s.querySelector('button[type="submit"]');i.disabled=!0;try{const o=p(await g("/api/auth/login",{method:"POST",body:{email:s.email.value.trim(),password:s.password.value}}));q(o.data),a.className="sb-message success",a.textContent=o.messages[0]||"Вход выполнен.",c(`Здравствуйте, ${o.data.fullName||o.data.FullName||"пользователь"}!`),h(),m?.(d()),m=null}catch(o){a.className="sb-message error",a.textContent=o.message}finally{i.disabled=!1}}),n.querySelector('[data-auth-form="register"]').addEventListener("submit",async r=>{r.preventDefault();const s=r.currentTarget,i=s.querySelector('button[type="submit"]');i.disabled=!0;try{const o=p(await g("/api/auth/register",{method:"POST",body:{firstName:s.firstName.value.trim(),lastName:s.lastName.value.trim(),email:s.email.value.trim(),password:s.password.value,phone:s.phone.value.trim(),address:s.address.value.trim(),passportNumber:s.passportNumber.value.trim()}}));a.className="sb-message success",a.textContent=o.messages[0]||"Регистрация завершена. Теперь войдите.",n.querySelector('[data-auth-tab="login"]').click(),n.querySelector("#sb-login-email").value=s.email.value.trim(),s.reset()}catch(o){a.className="sb-message error",a.textContent=o.message}finally{i.disabled=!1}}),n}function w(e="login",t=null){const a=k();m=t,a.hidden=!1,a.querySelector(`[data-auth-tab="${e}"]`)?.click()}function h(){n&&(n.hidden=!0)}function P(e,t="login"){if(v()){e(d());return}w(t,e)}function I(){const e=document.querySelector(".header-right");if(!e||e.querySelector(".sb-auth-controls"))return;const t=document.createElement("div");t.className="sb-auth-controls",e.appendChild(t);function a(){const r=d();if(!r){t.innerHTML=`
    ...
    `,t.querySelector('[data-action="logout"]').addEventListener("click",()=>y(!0))}a(),window.addEventListener("sbank:session-changed",a)}function B(e,t){return`<div class="sb-message ${e}">${t}</div>`}function _(e){const t=String(e||"").toLowerCase();return["active","completed","paid","success","read"].includes(t)?"success":["pending","reviewed","warning","new"].includes(t)?"warn":["blocked","closed","rejected","failed","forbidden"].includes(t)?"danger":""}function j(){return v()}const x=`${location.protocol}//${location.hostname}:5142`,T=localStorage.getItem("sbank-api-base-url")||window.SBANK_API_BASE_URL||x;export{T as A,g as a,B as b,O as c,_ as d,j as e,D as f,d as g,I as m,w as o,P as r,c as s,p as u};
